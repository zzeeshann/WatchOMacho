...
> **Status (2026-05-29):**
> - **Day-map v14 built 2026-05-29 (code-only, not deployed):** replaced the v13 SVG "comic" with a *map of the day* — but **not** the JSON+SVG shape originally drafted in [DAY_MAP_SPEC.md](DAY_MAP_SPEC.md) (now superseded). Instead it's **lab-shaped**: one LLM pass over the **full** briefing writes a single self-contained interactive HTML page (`planDayMap`; free-form HTML/CSS/JS, optionally one cdnjs lib), deliberately **concrete** (real names/dates/numbers — the inverse of daylila's abstract "labs"). Overview map of how the day connects, then the beats below. Stored in R2 (`day-maps/*.html`), linked from `reports.day_map_r2_key`/`day_map_slug` (v14 migration renames the v13 `comic_*` columns), served at `GET /day-map/:id` under a no-network CSP, embedded in a **sandboxed iframe** here and on daylila, and exposed on `/api/reports/:id` as `day_map {type:'html', html, slug, url}` (daylila's lab shape) + `/api/reports/recent` (slim `day_map {slug,url}`). No content validator — the iframe sandbox + CSP are the safety wall; the only check is "is this real HTML?". Toggle per-target or globally via `day_map_enabled` (default off). **Not yet deployed:** needs `wrangler d1 execute … migration-v14.sql` + `wrangler deploy`.

...
> **Status (2026-05-29):**
> - **Next up — Day-Map v2 (designed, not built):** replace the v13 linear "comic" with a richer "map of the day" — a structured graph (spine + nodes + cause→effect edges) exposed as JSON on the reports API **and** rendered to a hub-and-spoke SVG at its own URL, shown before the briefing on daylila. One LLM call, full briefing in, no Cloudflare AI. Full implementation spec (schema, prompt, layout, API/route/migration changes) in **[DAY_MAP_SPEC.md](DAY_MAP_SPEC.md)** — start there.
> - **Daily comic shipped 2026-05-29 (v13)** — after each briefing, an optional code-drawn SVG "connection comic" that captures the day's connecting thread (a *spine* + 3–5 cause→effect panels). One extra LLM pass (`planComic`) distils the finished briefing; `renderComicSvg()` lays it into a fixed connection-chain layout in brand colours — never an image-gen model. Lives in `makeComic()` (called from `runResearch`, so cron + manual both get it), best-effort so it never fails the briefing. Stored in R2 (`comics/*.svg`), linked from `reports.comic_r2_key`/`comic_slug` (v13 migration), served at `GET /comic/:id`, and exposed on `/api/reports/:id` (inline `comic.svg`) + `/api/reports/recent` (slim `comic {slug,url}`) for daylila. Toggle per-target on the target page or globally via the `comics_enabled` setting (default off).

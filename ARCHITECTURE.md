...
| 8 | **Day-map** (optional, v14) — when enabled for the target (`dayMapEnabledForTarget()`), one LLM pass over the **full** briefing → a self-contained interactive HTML "map of the day" (`planDayMap`; HTML/CSS/JS, optionally one cdnjs lib; concrete/news-specific). Prescribed layout: a **single vertical cause→effect column** (root driver → forces → effects, all ↓ connectors) at every width, DM Sans + daylila palette, tap-to-expand **inline** accordion, auto-sizes its iframe via `postMessage`. The only check is `looksLikeHtmlDoc()` (skip empty/truncated junk) — the sandboxed iframe + no-network CSP are the safety wall, not a content validator. Stored in R2 (`day-maps/*.html`) and linked from the report row (`day_map_r2_key`/`day_map_slug`). Best-effort, never fails the briefing. Runs in `runResearch` so cron + manual both get it; regenerable alone via the "Remake map" admin button. | 1 chat call (~16k out) + 1 R2 put + 1 D1 update | ~$0.02 (Sonnet) |
| 9 | **Lesson** (v15, 2026-06-01) — one LLM pass over the briefing body → the "why" under the news (the human/system pattern), as markdown. `makeLesson` (`src/contracts.ts` LESSON_CONTRACT + VOICE_CONTRACT, inline — no codegen). Own `# headline` + `> lede` parsed off; body stored R2 (`lessons/*.md`), row in `edition_pieces` kind='lesson'. Best-effort, never fails the briefing. Regenerable alone via `POST /admin/reports/:id/lesson`. | 1 chat call (~8k out) + 1 R2 put + 1 D1 upsert | ~$0.02 |
| 10 | **Lab** (v15, 2026-06-01) — one LLM pass over the lesson → a self-contained HTML "rehearse the decision" space (`makeLab`, LAB_CONTRACT). **Raw HTML like the day-map (NOT a JSON envelope)** — `extractHtmlDoc` + `validateLab` (sandbox-safety: no storage/network/eval/nested-iframe/forms, cdnjs allowlist, 50KB) + `looksLikeHtmlDoc`. Title/concept from a trailing `<!-- LAB title=…\|concept=… -->` comment; `<!-- LAB DECLINE -->` declines cleanly. Stored R2 (`labs/*.html`), row in `edition_pieces` kind='lab'. Best-effort. Regenerable alone via `POST /admin/reports/:id/lab`. | 1 chat call (~16k out) + 1 R2 put + 1 D1 upsert | ~$0.02 |

**Total per run (Sonnet default):** ~4 chat calls (write + day-map + lesson + lab) + 1 embedding + N Tavily calls (default N=10) → roughly **$0.08–0.50/run** + N Tavily credits. At 1 run/day = **~$2.50–15/month + 300 Tavily credits/month** (free tier is 1000). Haiku 4.5 selectable as a ~5× cheaper fallback.

> **Edition consolidation (v15, 2026-06-01):** WatchOMacho now creates the WHOLE edition — briefing + day-map + **lesson + lab** — and serves it all on `/api/reports/:id` (`lesson` + `lab` fields; `/recent` carries `has_lesson`/`has_lab` + slim metadata). Daylila reads + renders, generates nothing (its old lesson+lab pipeline is being retired). **`cronTick` now enqueues each due target into its `ResearchRunner` DO** (`scheduleManualRun(slug,'cron')`) rather than running `runResearch` inline — the 4-call edition runs inside the DO's 15-min budget + the `max_run_seconds` AbortController ceiling, so a hung call auto-aborts. Each piece is independently regenerable (DO `schedulePieceRun`) so one failure never forces a whole-edition rebuild.
...
day_map_enabled INTEGER            -- v14: day-map switch. 1 = on, 0 = off, NULL = inherit global `day_map_enabled` setting (ships 'off').
...
day_map_r2_key TEXT                -- v14: R2 key of the paired day-map HTML (`day-maps/*.html`). NULL = no day-map. (Renamed from v13 comic_r2_key; pre-v14 rows may still point at an old `.svg`.)
day_map_slug TEXT                  -- v14: short descriptive slug for the day-map's own page. NULL = no day-map.
created_at
```
...
| `day_map_enabled` | `off` | v14. Global default for the paired day-map step. Per-target `targets.day_map_enabled` overrides it. `dayMapEnabledForTarget()` resolves the effective value. |
...
day-maps/<created_at>-<report_id>.html   -- v14: paired day-map; self-contained interactive HTML (text/html)
static/tailwind.v4.css                    -- bundled CSS
```

The full markdown of every report (`text/markdown; charset=utf-8`) plus, when day-maps are enabled, the paired day-map HTML (`text/html`; v14). Never queried; only fetched by `r2_key` / `day_map_r2_key`. `findOrphanedR2()` treats both columns (and the `static/` keep-set) as referenced, so the GC sweep never deletes a live day-map.
...
| GET | `/api/reports/recent?limit=N` | Latest reports across targets (1–50, default 10). Slim — summary + source_count, no body. Includes `date` (ISO), `briefing_date` (v12: `YYYY-MM-DD` UTC — the canonical date for any UI that needs to label which day this briefing belongs to), and `day_map` (`{slug, url}` or `null`; v14 — slim, no inline HTML). |
| GET | `/api/reports/:id` | Full report — D1 metadata + R2 `body_markdown` + `sources[]` with `kind: "web" \| "archive"` + `day_map` (`{type:'html', html, slug, url}` with the interactive HTML inlined — daylila's lab shape — or `null`; v14). Same `briefing_date` field as `/recent`. |
